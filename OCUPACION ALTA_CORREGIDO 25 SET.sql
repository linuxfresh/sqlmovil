...
select 
 ...
 when TURNO <= '18:00' then 'TARDE' 
  ...
  END) TURNO,turno HORA,--SERVICIO,
fecha_partida,o_d, SERVICIO,SUM(capacidad_bus) CAPACIDAD_BUS, SUM(t_boleto) T_BOLETO, round(SUM(t_boleto)/SUM(capacidad_bus),2) ocup,round(t_monto/KMS,2) SOL_KM
from
(
select distinct --ptmy.EMPRESA,  
                 ptmy.FECHA_PARTIDA,  
                 --TO_CHAR(to_date(to_char(sysdate,'YYYY-MM-DD')||ptmy.TURNO, 'dd/mm/yyyy hh24:mi:ss'),'HH24:MM') HORA,  
									ptmy.TURNO,
                ...
                TO_CHAR(count(1)) T_BOLETO, 
								...
								sum(ptmy.T_MONTO) T_MONTO  ,ptmy.KMS KMS
                from (SELECT  E.C_NOMCOR EMPRESA,     
                TO_CHAR(I.D_FECPAR,'YYYY-MM-DD') FECHA_PARTIDA,  
                I.C_HORPAR TURNO,          
                REPLACE(R.C_ORIGEN,'PUERTO','PTO') ORIGEN,  
                REPLACE(R.C_DESTINO,'PUERTO','PTO') DESTINO,r.n_kilometros kms,   
								DECODE(S.C_NOMCOR,
                ...
                where v.tipmov_id not in( 5,6,13,14)
								AND r.ruta_id not in (630,602,594,629,21,22,292,293,38,47,318,309)   
                AND v.c_estreg='A'    --and i.TIPITI_ID = 1 
								AND I.N_ESANULADO = 0     
                and --(v.d_fecpar between to_date('27/01/2018','YYYY-MM-DD') and to_date('27/01/2018','YYYY-MM-DD')
											 --or 
											 --v.d_fecpar between to_date('28/01/2019','YYYY-MM-DD') and to_date('03/02/2019','YYYY-MM-DD')--)
											 (i.D_FECPAR between sysdate-1 and sysdate+31)
                and v.agencia_id <> 69) ptmy        
                group by  --ptmy.EMPRESA,     
                     ptmy.FECHA_PARTIDA,  
                     --TO_CHAR(to_date(to_char(sysdate,'YYYY-MM-DD')||ptmy.TURNO, 'dd/mm/yyyy hh24:mi:ss'),'HH24:MM'), 
											ptmy.TURNO,
                     ptmy.ORIGEN||' - '||ptmy.DESTINO,   
                     ptmy.SERVICIO,   ptmy.KMS,
                     --ptmy.NRO_BUS,  
                     --ptmy.PLACA_BUS,
-- 										 ptmy.DNITERRAMOZA,  
--                      ptmy.TERRAMOZA,  
...
GROUP BY (case
 ...
 when TURNO <= '18:00' then 'TARDE' 
  ...
  END),fecha_partida,o_d,turno,round(t_monto/KMS,2),SERVICIO
HAVING round(SUM(t_boleto)/SUM(capacidad_bus),2)>=0 and SUBSTR(TURNO,5,1) not in ('1','2','3','4') and ((FECHA_PARTIDA>TO_CHAR(SYSDATE,'YYYY-MM-DD')) or (FECHA_PARTIDA=TO_CHAR(SYSDATE,'YYYY-MM-DD') and TURNO>to_char(sysdate, 'HH24:MI:SS')))
...
select distinct --ptmy.EMPRESA,  
                ...
                LEFT JOIN VRMPERSONAL   PE ON PE.PERSONAL_ID  = PROS.PERSONAL_IDPILOTO  
                LEFT JOIN  VRMPERSONAL   CO ON CO.PERSONAL_ID  = PROS.PERSONAL_IDCOPILOTO  
                where v.tipmov_id not in( 5,6,13,14,11)
...
from
(
SELECT DISTINCT TO_CHAR(i.D_FECPAR,'YYYY-MM-DD') fecha_PARTIDA, 
...
left join (SELECT * from VRTVENPAS where tipmov_id not in( 5,6,13,14,11) AND 
...
where (i.D_FECPAR between sysdate-1 and sysdate+31)																	 --to_date('31/01/2019','YYYY-MM-DD'))
--OR (i.D_FECPAR between to_date('27/12/2017','YYYY-MM-DD') and to_date('27/12/2018','YYYY-MM-DD'))
and i.C_ESTREG = 'A' 
--and t.C_ESTREG = 'A' 
--and i.TIPITI_ID = 1 
AND I.N_ESANULADO = 0
