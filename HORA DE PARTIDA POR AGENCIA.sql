select distinct --ptmy.AUDUSUINS,ptmy.AUDUSUMOD,--ptmy.EMPRESA,  
                 ptmy.FECHA_PARTIDA, ptmy.HORA_PART,ptmy.AGENCIA_PARTIDA,
                ptmy.TURNO,      
                 ptmy.ORIGEN||' - '||ptmy.DESTINO O_D,-- ptmy.ITINERARIO_ID,  
                ...
                from (SELECT --I.AUDUSUINS,I.AUDUSUMOD, --E.C_NOMCOR EMPRESA,     
...
--                 LEFT JOIN  VRMPERSONAL   CO ON CO.PERSONAL_ID  = PROS.PERSONAL_IDCOPILOTO
                ...
                where --v.tipmov_id not in( 5,6,13,14)AND 
								--r.ruta_id not in (630,602,594,629)
               ...
               and I.N_ESANULADO = 0 AND
								(I.d_fecpar between to_date('30/10/2019','dd/mm/yyyy') and to_date('31/10/2019','dd/mm/yyyy'))
											--or v.d_fecpar between to_date('01/07/2019','dd/mm/yyyy') and to_date('1/08/2019','dd/mm/yyyy'))
                --and v.agencia_id <> 69
								) ptmy        
                group by --ptmy.AUDUSUINS,ptmy.AUDUSUMOD, --ptmy.EMPRESA,     
                     ptmy.FECHA_PARTIDA,  	
                     ptmy.TURNO, ptmy.ORIGEN,ptmy.DESTINO,
										 ptmy.HORA_PART,ptmy.AGENCIA_PARTIDA,          
                     ptmy.ORIGEN||' - '||ptmy.DESTINO, --  ptmy.ITINERARIO_ID,
                     ptmy.SERVICIO,   
                     --ptmy.NRO_BUS,  
                     --ptmy.PLACA_BUS,
			...
			--AND ptmy.destino = 'LA MERCED'
			--HAVING ptmy.AGENCIA_PARTIDA ='JAVIER PRADO-TERM' OR ptmy.AGENCIA_PARTIDA ='TOMAS VALLE TERRA'
			ORDER BY HORA_PART